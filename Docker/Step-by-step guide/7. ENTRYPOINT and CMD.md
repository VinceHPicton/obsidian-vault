...
In our image, the only thing you can do is run the binary we made, so we're going to force the image to only ever be able to start a container with that command. We'll do this with the ENTRYPOINT instruction.
